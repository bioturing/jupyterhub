...
c.JupyterHub.spawner_class = "kubespawner.spawner.BioTuringKubeSpawner"
...
common_labels = c.BioTuringKubeSpawner.common_labels = {}
...
for trait, cfg_key in (
    ...
    if cfg_key is None:
        cfg_key = camelCaseify(trait)
    set_config_if_not_none(c.BioTuringKubeSpawner, trait, "singleuser." + cfg_key)
...
if image:
    ...
    c.BioTuringKubeSpawner.image = image
...
if image_pull_secrets:
    c.BioTuringKubeSpawner.image_pull_secrets = image_pull_secrets

# scheduling:
if get_config("scheduling.userScheduler.enabled"):
    c.BioTuringKubeSpawner.scheduler_name = get_name("user-scheduler")
if get_config("scheduling.podPriority.enabled"):
    c.BioTuringKubeSpawner.priority_class_name = get_name("priority")
...
if match_node_purpose:
    ...
    if match_node_purpose == "prefer":
        c.BioTuringKubeSpawner.node_affinity_preferred.append(
            dict(
                weight=100,
        ...
        c.BioTuringKubeSpawner.node_affinity_required.append(node_selector)
...
if tolerations:
    c.BioTuringKubeSpawner.tolerations = tolerations
...
if storage_type == "dynamic":
    ...
    set_config_if_not_none(
        c.BioTuringKubeSpawner,
        "storage_access_modes",
        "singleuser.storage.dynamic.storageAccessModes",
    )
    set_config_if_not_none(
        c.BioTuringKubeSpawner, "storage_capacity", "singleuser.storage.capacity"
    )
...
if extra_files:
    ...
    c.BioTuringKubeSpawner.volumes.append(volume)
    ...
    c.BioTuringKubeSpawner.volume_mounts.extend(volume_mounts)

# Inject extraVolumes / extraVolumeMounts
c.BioTuringKubeSpawner.volumes.extend(get_config("singleuser.storage.extraVolumes", []))
c.BioTuringKubeSpawner.volume_mounts.extend(
...
if cloud_metadata.get("blockWithIptables") == True:
    ...
    c.BioTuringKubeSpawner.init_containers.append(ip_block_container)
